...
class TestRestServer(TestCase):
    ...
    @mock.patch('logic.requests.post')
    def test_cash_purchase(self, post_mock):
        post_mock.return_value.status_code = 200
        post_mock.return_value.text =\
            '{"products": [{"slot": 0, "count": 5, "price": 80, "id": 0, "name": "snickers"}]}'
        data = json.dumps({"filler_url": "url1", "charger_url": "url2"})
    ...
    @mock.patch('logic.requests.post')
    def test_card_purchase(self, post_mock):
        res1 = mock.MagicMock()
        res1.status_code = 200
        res1.text = '{"products": [{"slot": 0, "count": 5, "price": 80, "id": 0, "name": "snickers"}]}'
        res2 = mock.MagicMock()
        res2.status_code = 200
        res2.text = '{"success": true}'
        post_mock.side_effect = [res1, res2]
        data = json.dumps({"filler_url": "url1", "charger_url": "url2"})
        res = self.app.post('/init', data=data, content_type='application/json')
        self.assertEqual(res.status_code, 200)
        self.assertEqual(post_mock.call_count, 1)
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(res.data, '{"credit": 100}')
        data = json.dumps({"slot": 0, "payment": "card", "payment_details": {"ccnum": "1234567891234567", "pin": "1000"}})
        res = self.app.post('/purchase', data=data, content_type='application/json')
        self.assertEqual(res.data, '{"credit": 100, "result": "success"}')
